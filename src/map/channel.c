...
int channel_join(struct Channel *channel, struct map_session_data *sd) {
	...
	if (channel->type == CHAN_TYPE_PRIVATE && db_size(channel->users) >= channel_config.private_channel.max_member) {
		char output[CHAT_SIZE_MAX];
		sprintf(output, msg_txt(sd,881), channel->name, channel_config.private_channel.max_member); // Cannot join channel '%s'. Reached limit of private channel %d.
		clif_displaymessage(sd->fd, output);
		return -4;
	}

	if (channel_pcisjoined(channel, sd))
		return 0;
	...
	if( sd->stealth ) {
		...
		safesnprintf(output, CHAT_SIZE_MAX, msg_txt(sd,882), channel->alias, sd->status.name); // %s %s has joined
 ...
 * @flag&2 : join the channel if not joined yet (map or ally only)
 * return
...
int channel_pcleave(struct map_session_data *sd, char *chname){
	...
	if (!(channel->opt&CHAN_OPT_CAN_LEAVE)) {
		sprintf(output, msg_txt(sd,883) ,chname); // You cannot leave '%s' channel.
		clif_displaymessage(sd->fd, output);
		return -1;
	}
	...
	if( !channel_config.closing && (channel->opt & CHAN_OPT_ANNOUNCE_LEAVE) ) {
		char output[CHAT_SIZE_MAX+1];
		safesnprintf(output, CHAT_SIZE_MAX, msg_txt(sd,884), channel->alias, sd->status.name); // %s %s left
		clif_channel_msg(channel,output,channel->color);
	}
	switch(channel->type){
...
int channel_pccolor(struct map_session_data *sd, char *chname, char *color){
	...
	if( !pc_has_permission(sd, PC_PERM_CHANNEL_ADMIN) ) {
		if (channel->char_id != sd->status.char_id) {
			...
			sprintf(output, msg_txt(sd,885), chname); // Cannot set color_override for this channel '%s'.
			clif_displaymessage(sd->fd, output);
			return -1;
		}
	}
...
int channel_pcban(struct map_session_data *sd, char *chname, char *pname, int flag){
	...
	if( !pc_has_permission(sd, PC_PERM_CHANNEL_ADMIN) ) {
		if (channel->char_id != sd->status.char_id) {
			...
			sprintf(output, msg_txt(sd,886), chname); // Not allowed to ban a player.
			clif_displaymessage(sd->fd, output);
		}
		return -1;
	}
...
int channel_pckick(struct map_session_data *sd, char *chname, char *pname, int flag) {
	...
	if( !pc_has_permission(sd, PC_PERM_CHANNEL_ADMIN) ) {
		if (channel->char_id != sd->status.char_id) {
			...
			sprintf(output, msg_txt(sd,887), chname); // Not allowed to kick a player.
			clif_displaymessage(sd->fd, output);
		}
		return -1;
	}
	...
	if (channel_pc_haschan(sd, channel) < 0) {
		sprintf(output, msg_txt(sd,888), pname, chname); // Player '%s' not join channel '%s'.
		clif_displaymessage(sd->fd, output);
		return -1;
	}

	if (channel->char_id == sd->status.char_id) {
		clif_displaymessage(sd->fd, msg_txt(sd, 888)); // Not allowed to kick a player.
		return -1;
	}

	if( !channel_config.closing && (channel->opt & CHAN_OPT_ANNOUNCE_LEAVE) ) {
		char output[CHAT_SIZE_MAX+1];
		safesnprintf(output, CHAT_SIZE_MAX, msg_txt(sd,889), channel->alias, tsd->status.name); // %s %s is kicked
		clif_channel_msg(channel,output,channel->color);
	}
...
int channel_pcisjoined_sub(DBKey key, DBData *data, va_list ap) {
	unsigned int char_id = (unsigned int)db_data2ptr(data);
	struct map_session_data *sd = NULL;

	nullpo_ret(char_id);
	nullpo_ret((sd = va_arg(ap, struct map_session_data *)));

	if (char_id == sd->status.char_id)
		return 1;
...
bool channel_pcisjoined(struct Channel *channel, struct map_session_data *sd) {
	...
	if (channel->users)
		return channel->users->foreach(channel->users, channel_pcisjoined_sub);
...
bool channel_read_sub(config_setting_t *chan, struct Channel *tmp_chan, uint8 i) {
	...
	char *name = NULL, *password = NULL, *alias = NULL, *color_str = "Default", *type_str = NULL;

	if (tmp_chan == NULL)
		return false;

	//config_setting_lookup_int(chan, "id", &id); // Unused yet
