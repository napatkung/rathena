...
BUILDIN_FUNC(channel_ban) {
	...
	if (!idb_exists(ch->banned, char_id)) {
		...
		if (tsd) {
			...
			safesnprintf(output, CHAT_SIZE_MAX, msg_txt(tsd,890), ch->alias, tsd->status.name); // "%s %s is banned"
		}
		else
			safesnprintf(output, CHAT_SIZE_MAX, msg_txt(tsd,890), ch->alias, "****"); // "%s %s is banned"
...
BUILDIN_FUNC(channel_unban) {
	...
	if (idb_exists(ch->banned, char_id)) {
		...
		if (tsd)
			safesnprintf(output, CHAT_SIZE_MAX, msg_txt(tsd,891), ch->alias, tsd->status.name); // "%s %s is unbanned"
		else
			safesnprintf(output, CHAT_SIZE_MAX, msg_txt(tsd,891), ch->alias, "****"); // "%s %s is unbanned"
